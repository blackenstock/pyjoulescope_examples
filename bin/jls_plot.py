...
def run():
    ...
    for divisor in [9,8,7,6,4]:
        incr = d_idx // (d_idx // divisor) #args.sample_count
        ...
        for i in range(1,220,1):
            peak_eco_readout, peak_height_eco_readout = find_peaks(xs, height=(0.38,5), distance=i) # 100us width > 200mA (this level is only reached from ECO peak): 100us/(1/sample_frequency)=100us/(1/2000000)=200 > 200/sample_count=200/i >
            if(0 < peak_eco_readout.__len__()):
                print("incr,divisor:"+str(incr)+","+str(divisor)+" peaks found for distance:"+str(i)+" peak_eco_readout_length:"+str(peak_eco_readout.__len__()))
                cnt+=1
        if(cnt==0):
            print("no peaks found incr:"+str(incr))
        if(cnt==3):
            print("correct number of peaks found!:"+str(incr))
        #print("peaks at:{}",peak2s)
        #region
        #cnt=0
        #for peak in peak2s:
        #    print("sample: ",peak)
